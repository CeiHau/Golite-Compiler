...
func NewCmp(result int, cond string, op1 *Operand, op2 *Operand) *Cmp {
	return &Cmp{result, cond, op1, op2}
}
...
func (instr *Cmp) GetCondCode() string {
	return instr.cond
}

func (instr *Cmp) TranslateToAssembly(tables st.SymbolTables, armTable map[string]string) []codegen.Instruction {
	...
	res = codegen.WriteCode(res, fmt.Sprintf("cmp %v, %v", reg2.Str, reg1.Str))
	// res = codegen.WriteCode(res, fmt.Sprintf("b.%v %v", instr.cond))
	reg1.FreeRegister()
	reg2.FreeRegister()
	return res
}
