...
func NewAnd(result int, op1 *Operand, op2 *Operand) *And {
...
func (instr *And) TranslateToAssembly(tables st.SymbolTables, armTable map[string]string) []codegen.Instruction {
	targetReg := fmt.Sprintf("r%d", instr.result)
	...
	res = codegen.WriteCode(res, fmt.Sprintf("and %v, %v, %v", reg3.Str, reg2.Str, reg1.Str))
