...
// Sub represents a SUB instruction in ILOC
type Sub struct {
...
// NewSub is a constructor and initialization function for a new Sub instruction
func NewSub(result int, op1 *Operand, op2 *Operand) *Sub {
	return &Sub{result, op1, op2}
}
...
func (instr *Sub) TranslateToAssembly(tables st.SymbolTables, armTable map[string]string) []codegen.Instruction {
	targetReg := fmt.Sprintf("r%d", instr.result)
	...
	res = codegen.WriteCode(res, fmt.Sprintf("sub %v, %v, %v", reg3.Str, reg1.Str, reg2.Str))
